...
import type {
  IServiceDataKeys,
  IServicesDashboards,
  IServicesLastBillPages,
  IServicesLastBillSelectors,
  IServicesLoginPages,
  IServicesLoginSelectors,
  IServicesStatuses,
} from "#/types/api";
...
type ApiRes<T> =
  ...
  | { key: IServiceDataKeys; data: null; error: string };
...
const API = <T>(key: IServiceDataKeys, nextURL: string): Promise<ApiRes<T>> =>
  fetch(`${TypedEnv.backend_endpoint}/${nextURL}`)
    .then(async (x) => {
      SequenceUtilities.DEBUG_MODE &&
        log.warning(`[DEBUG] ${key}: ${JSON.stringify(x)}`);
...
export const ServerEndpoint = {
  ...
  getServicesLoginSelectors: () =>
    API<IServicesLoginSelectors>("login-selectors", "services/login-selectors"),
  getServicesDashboardPages: () =>
    API<IServicesDashboards>("dashboard-url", "services/dashboards-url"),
  getServicesLastBillPages: () =>
    API<IServicesLastBillPages>("last-bill-url", "services/last-bill-pages"),
  getServicesLastBillSelectors: () =>
    API<IServicesLastBillSelectors>(
      "last-bill-selectors",
      "services/last-bill-selectors",
    ),
};
