...
export class ZodSchemaManager<LastVer extends ZodSemverUnbranded, Instance extends any> {
  ...
  migrate<T>(data: T, from: IValidVersions<Instance>, to: IValidVersions<Instance>): z.SafeParseReturnType<T, T> {
    let currentData: z.SafeParseReturnType<T, T> = {success:true,data}
    let currentVersion = from;

    const sortedVers = [...this.versions]
      .map((x) => x[0])
      .sort(semver.compare);
